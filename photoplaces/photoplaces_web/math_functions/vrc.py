...
def vrc(run, N):
    ...
    month_cycle = run.clusters.all()[0].photos.all()[0].normalized_entry.get().normalized_set.month_z_cycle_length
    cluster_means_x = []
    cluster_means_y = []
    # cluster_means_month = []
    cluster_n = []
    ...
    for cluster in all_clusters:
        ...
        ssw += np.sum((points_x - m_x) ** 2 + (points_y - m_y) ** 2 )#+ cyclical_distance(points_month, m_month, month_cycle) ** 2)
    ...
    # global_mean_month = cycle_avg(all_points_month, month_cycle)
    ...
    ssb = np.sum(cluster_n * ((cluster_means_x - global_mean_x) ** 2 + (cluster_means_y - global_mean_y) ** 2 ))#+ cyclical_distance(global_mean_month, cluster_means_month, month_cycle) ** 2))
